...
do
    ...
    function Funcs:AddSpacer(Height)
        ...
        local SpacerFrame = Library:Create('Frame', {
            Size = UDim2.new(1, -4, 0, Height or 10); 
            BackgroundTransparency = 1; 
            Parent = Container;
            ZIndex = 5;
        });
    
        Spacer.Frame = SpacerFrame;
        Spacer.Container = Container;

        function Spacer:SetHeight(newHeight)
            SpacerFrame.Size = UDim2.new(1, -4, 0, newHeight);
            Groupbox:Resize();
        end
    ...
    function Funcs:AddDivider()
        ...
        local DividerStroke = Library:Create('UIStroke', {
            Color = Library.OutlineColor;
            Thickness = 1;
            Parent = DividerOuter;
        });

        Library:AddToRegistry(DividerStroke, {
            Color = 'OutlineColor';
        });
        ...
        Library:OnHighlight(DividerOuter, DividerOuter,
            { BorderColor3 = 'AccentColor' },
            { BorderColor3 = 'Black' }
        );
...
do
    ...
    local WatermarkOuter = Library:Create('Frame', {
        ...
        Position = UDim2.new(0.5, 0, 0, -48); -- 8 pixels from the top
    ...
    local BackgroundCorner = Library:Create('UICorner', {
        CornerRadius = UDim.new(0, 4),
        Parent = WatermarkInner
    });
...
function Library:CreateWindow(...)
    ...
    if type(...) == 'table' then
        ...
        Config.Version = Arguments[2]
        Config.AutoShow = Arguments[3] or false;
    end

    if type(Config.Title) ~= 'string' then Config.Title = 'No title' end
    if type(Config.Version) ~= 'string' then Config.Version = '' end
    if type(Config.MenuFadeTime) ~= 'number' then Config.MenuFadeTime = 0.2 end

    if typeof(Config.Position) ~= 'UDim2' then Config.Position = UDim2.fromOffset(175, 50) end
    if type(Config.Size) ~= 'UDim2' then Config.Size = UDim2.fromOffset(550, 570) end
    ...
    local newSize = UDim2.new(
        Config.Size.X.Scale, 
        Config.Size.X.Offset + 57,
        Config.Size.Y.Scale, 
        Config.Size.Y.Offset + 57
    );
    ...
    local WindowLabel = Library:CreateLabel({
        ...
        Text = if Config.Version and Config.Version ~= "" then Config.Title .. " | v" .. Config.Version else Config.Title;
    ...
    local TabAreaCorner = Library:Create('UICorner', {
        CornerRadius = UDim.new(0, 4),
        Parent = TabArea
    });
    ...
    Library:OnHighlight(TabContainer, TabContainer,
    { BorderColor3 = 'AccentColor' },
    { BorderColor3 = 'OutlineColor' }
    );
    ...
    function Window:AddTab(Name)
        ...
        function Tab:AddGroupbox(Info)
            ...
            Library:AddToRegistry(BoxInner.UIStroke, {
                Color = 'OutlineColor';
            });
            Library:OnHighlight(BoxInner, BoxInner,
                { BorderColor3 = 'AccentColor' },
                { BorderColor3 = 'OutlineColor' }
            );
            Library:Create('UICorner', {
